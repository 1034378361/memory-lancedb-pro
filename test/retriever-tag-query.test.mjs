...
describe("MemoryRetriever - Tag Query", () => {
  describe("BM25-only retrieval with mustContain", () => {
    ...
    it("should match tags case-insensitively", async () => {
      ...
      assert.equal(results.length, 2, "Should match both case variants");
      assert.ok(
        results.some((r) => r.entry.id === "1"),
        "Should match lowercase proj:aif"
      );
      assert.ok(
        results.some((r) => r.entry.id === "2"),
        "Should match uppercase proj:AIF"
      );
    });
